...
class CounterfactualBunchingAnalysis:
    ...
    def __init__(self, threshold=85, window_left=10, window_right=10, 
    ...
    def load_and_prepare_data(self):
        ...
        # Load data
        data_path = Path(__file__).parent / 'synthetic_firms_85k.csv'
    ...
    def run_analysis(self, tau_e=None, include_step4=False, **wedge_params):
        ...
        if include_step4:  # Only do Step 5 if we're doing the full analysis
            self.step5_forward_map_to_new_policy(T_star_new=95)
    ...
    def plot_results(self, include_step5=False):
        ...
        ax.plot(y_dist_plot, self.f_obs[plot_mask], 'o-', color='blue', markersize=3, 
                linewidth=1, label='Original Observed f^obs(T) [£85k]', alpha=0.7)
        ...
        # Add vertical lines  
        ax.axvline(x=0, color='black', linestyle='-', linewidth=2, label='Original Threshold £85k')
        ax.axvline(x=-self.W_left, color='gray', linestyle='--', alpha=0.5, label=f'£85k -£{self.W_left}k/+£{self.W_right}k')
        ...
        # Formatting
        ax.set_xlabel('Distance from Original Threshold £85k (£1,000)', fontsize=12)
        ...
        # Save plot
        output_path = Path(__file__).parent / 'bunching_analysis_85k.png'
