...
import os
THRESHOLD = float(os.environ.get('VAT_THRESHOLD', '90'))  # VAT threshold in £k
...
class SyntheticFirmGenerator:
    ...
    def assign_vat_registration_flags(self, turnover_values: Tensor, hmrc_bands: Dict[str, int]) -> Tensor:
        ...
        synthetic_1_to_threshold = ((turnover_values > 0) & (turnover_values <= THRESHOLD)).sum().item()  # Current synthetic count
        ...
        mandatory_vat = turnover_values > THRESHOLD
        
        # Voluntary registration below threshold (but above 0)
        below_threshold = (turnover_values > 0) & (turnover_values <= THRESHOLD)
    ...
    def _map_to_hmrc_bands(self, turnover_values: Tensor) -> Tensor:
        ...
        band_indices = torch.where((turnover_values > 0) & (turnover_values <= THRESHOLD), 1, band_indices)  # £1_to_Threshold
        band_indices = torch.where((turnover_values > THRESHOLD) & (turnover_values <= 150), 2, band_indices)  # £Threshold_to_£150k
...
def main():
    ...
    # Save results
    output_path = Path(__file__).parent / f'synthetic_firms_{int(THRESHOLD)}k.csv'
